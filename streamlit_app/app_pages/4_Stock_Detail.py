...
st.caption("Gemini 2.5 Flash로 실적 품질 분석 · 수동 트리거 · 결과는 투자 조언이 아닙니다.")

if st.button("Generate AI Earnings Summary", key="ai_earnings_btn", type="primary"):
    with st.spinner("Analyzing earnings data with Gemini 2.5 Flash…"):
        _gclient = get_gemini_client()
        if _gclient is None:
            st.warning("GEMINI_API_KEY not configured in .streamlit/secrets.toml")
        else:
            _earn_context = {
                "ticker": selected,
                ...
                "forward_eps": _val_core.get("forward_eps") if _val_core else None,
                "revenue_growth_yoy": _val_core.get("revenue_growth_yoy") if _val_core else None,
                "gross_margin": _val_core.get("gross_margin") if _val_core else None,
                "analyst_target_mean": _consensus.get("target_mean") if _consensus else None,
            }
            if not _earn_df.empty:
...
Keep it factual and under 300 words. Use markdown formatting."""
            ...
            try:
                ...
                _ai_summary = (_resp.text or "").strip()
                st.markdown(
                ...
                st.caption("Powered by Gemini 2.5 Flash · Not investment advice")
